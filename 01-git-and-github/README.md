...
先按 `Win+E` 打开 Windows 文件资源管理器，开启扩展名显示：Windows 11 通常在“查看 → 显示 → 文件扩展名”；Windows 10 通常在“查看”选项卡勾选“文件扩展名”。这样能看出 `index.html` 和 `index.html.txt` 的区别，后者仍然是 TXT 文件。
...
3. 单击资源管理器的地址栏，查看它所在的位置。类似 `C:\Users\你的用户名\Documents\code-practice\my-first-page` 的文字叫**路径**，它说明文件夹在哪里。你的实际路径可能不同，不要照抄这个示例。
...
代码块前标注 `powershell`，表示这些内容应在终端执行；不要把它们粘进网页文件。复制时只复制命令本身，不复制代码围栏、`PS C:\...>` 这样的提示符或已经打印出来的结果。
...
用户名会出现在个人主页和仓库地址中，建议使用容易辨认、长期可用的英文名称。密码不要与其他重要网站共用，并建议开启两步验证。
...
如果 GitHub 页面无法打开或加载很慢，可以在遵守所在地法律法规、学校规定和网络安全要求的前提下，使用合规方式进行科学上网。这里只提醒网络可达性问题，不介绍具体工具。无论使用何种网络，都不要安装来路不明的软件，也不要向陌生人提供账号、验证码或密钥。
...
3. 选择 HTTPS，复制仓库地址。
4. 在电脑上创建一个专门存放项目的目录。
5. 在 VS Code 中打开该目录，并打开集成终端。
6. 执行 `git clone 仓库地址`。
7. 在 VS Code 中打开刚下载的项目文件夹。

`clone` 会自己新建项目子文件夹，下载前终端应位于用于存放项目的父目录。下载结束后，可以用 VS Code 的“打开文件夹”进入它，再新建终端。不要在同一位置反复 clone；先检查第一次是否已经生成了文件夹。
...
（图片 009：推送分支后出现的 Compare & pull request 提示）

（图片 010：新建 Pull Request 页面，标出源分支、目标分支、标题与说明）
...
你在实验开始时只需要使用 [webpage-source.txt](webpage-source.txt)。它包含完整的 HTML 源码，里面没有需要删除的说明文字或 Markdown 围栏。

面向学生的实验材料不提前提供或链接可直接打开的 `index.html`。你需要在第 0 节创建的 `code-practice/my-first-page/` 中亲手创建它，再把 TXT 的全部内容粘贴进去。保存并用浏览器打开后，才会第一次看到完整网页。这正是本实验要练习的过程。

开始复制前，你自己的项目应该是一个空文件夹：
...
（图片 039：VS Code 中左侧项目内的 index.html 和项目外打开的 TXT 标签，突出正确文件名与 HTML 语言模式，不要求截图完整源码）
...
7. 看到“你，未被定义。可能，正在发生。”和粒子环，就说明网页打开成功。按下面的小任务探索它：
   ...
   - 移动“秩序 / 自由”滑块，观察形态变化；点击三个色圆，选择青柠、落日或冰蓝氛围。
...
（图片 040：新版“未定义”网页的首屏整体效果，展示粒子作品、主标题以及配色和形态控制）

（图片 041：展开一项心情回应后的页面，展示选中区域变宽、背景变色和文字内容）

（图片 042：宣言卡弹窗与浏览器下载列表，展示虚构昵称、实时预览和生成的 PNG 文件）
...
（图片 013：仓库 Settings 中 Pages 的入口）

（图片 014：Pages 部署设置，标出 main 分支、根目录和 Save）

（图片 015：Pages 部署成功后显示的网址）
...
- 自己从 TXT 创建了 `index.html`，能够解释它和 `index.html.txt` 的区别。
- 仓库根目录中存在 `index.html`，没有把教程整套上传。
- 标题与署名至少修改了两处，刷新后仍保留。
- 粒子拖动、打散、形态和配色调节均可以操作，暂停和重置有效。
- 心情区域可以展开，宣言卡可以实时预览，下载的 PNG 能正常打开且文字完整。
- 提交信息能够说明修改内容。
...
## 14. 实验二：完成一次 GitHub 协作

### 实验背景

老师提供一个“班级介绍页”仓库。每位学习者添加一张自己的介绍卡片，通过 PR 交给老师合并。不要直接修改主分支。
...
3. 建立 `add-你的英文名` 分支。
4. 按项目说明，在指定文件中增加姓名、兴趣和一句学习目标，不修改别人的内容。
5. 在本地打开网页检查格式。
6. 使用 `git status` 检查改动，完成 Commit 和 Push。
7. 从自己的分支向原仓库的 `main` 分支发起 PR。
8. 在 PR 描述中写清修改内容和检查方法。
9. 根据老师的一条审查意见修改，再次 Commit 和 Push。
...
（图片 016：协作实验中填写完成的 PR 页面，突出标题和说明）

（图片 017：PR 的 Files changed 页面，展示如何检查实际改动）

（图片 018：收到审查意见后 PR 对话区的示例）

### 验收标准

- 修改发生在独立分支中。
- 只修改任务要求的内容，没有覆盖同学的代码。
- PR 的源仓库、源分支和目标分支正确。
- 提交信息清楚，PR 说明完整。
- 能根据反馈更新同一个 PR。
