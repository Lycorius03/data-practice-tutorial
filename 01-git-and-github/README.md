...
![图片 035：Windows 文件资源管理器中开启“文件扩展名”的位置，展示 index.html 与 index.html.txt 的区别](../image/035.png)
...
![图片 036：资源管理器中的 code-practice 和 my-first-page 文件夹，框出地址栏与项目文件夹](../image/036.png)
...
![图片 037：VS Code 打开文件夹后的页面，标出项目名称、新建文件按钮与编辑区](../image/037.png)
...
![图片 038：VS Code 的终端菜单和 PowerShell 终端位置，标出当前路径；无需单独截图命令输出](../image/038.png)
...
![图片 001：GitHub 首页，框出注册和登录入口](../image/001.png)

![图片 002：GitHub 注册页面，标出邮箱、密码和用户名填写位置，注意遮挡个人信息](../image/002.png)

### 关于网络访问

GitHub 需要正常联网。如果页面打不开或加载很慢，请先检查网络连接。必要时，请在遵守所在地法律法规、学校规定和网络安全要求的前提下，使用合规方式科学上网。本教程不讲具体配置，也不要安装来路不明的软件，或向他人提供账号、验证码和密钥。

![图片 009：网络访问提示或 Watt Toolkit 页面，截图时不要包含账号、验证码和个人信息](../image/Watt%20Toolkit.png)
...
![图片 003：登录后的 GitHub 首页，标出个人头像和仓库入口](../image/003.png)

![图片 004：一个示例仓库的主页，标出 Code、Issues、Pull requests、Actions 和 Settings](../image/004.png)
...
![图片 005：Git 官方 Windows 下载页面，标出正确的下载入口](../image/005.png)
...
![图片 006：仓库 Code 按钮展开后的 HTTPS 地址复制位置](../image/006.png)
...
![图片 007：原仓库页面右上角的 Fork 按钮](../image/007.png)

![图片 008：创建 Fork 页面，标出仓库名称和 Create fork 按钮](../image/008.png)
...
3. 本次练习不要自动创建 README、`.gitignore` 或 License，让远程仓库保持为空，再单击 **Create repository**。
   ...
   ![图片 011：GitHub 右上角加号菜单中的 New repository](../image/011.png)

   ![图片 012：创建仓库页面，标出仓库名、Public、空仓库相关选项和 Create repository](../image/012.png)
